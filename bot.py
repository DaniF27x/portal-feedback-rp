...
CARGOS\_STAFF = { 'CEO': 1365557796879466566, 'Presidente': 1365769992246788106, 'Master': 1366312318392078356, 'Alta Cúpula': 1365518838472376422, 'ADM staff': 1365519563948687472, 'Resp Comunidade': 1365518494958882866, 'Mentoria': 1367280730572460113, 'Equipe Legal': 1365518739826671707, 'Social Midia': 1365519199555817634, 'ADM': 1374047506257678356 }

CARGO\_CIDADAO = 1365520018124570667

# 🎯 CANAL ESPECÍFICO PARA TICKETS

CANAL\_FEEDBACK\_ID = 1365522389827326013 # SEU CANAL DE FEEDBACK

# CARGOS AUTOMÁTICOS POR CATEGORIA

CATEGORIAS = { 'melhorias\_cidade': { 'nome': '🏗️ Melhorias na Cidade', 'emoji': '🏗️', 'descricao': 'Ideias para locais, construções, mapas', 'cargos': \['CEO', 'Presidente', 'Master', 'Alta Cúpula', 'ADM staff', 'ADM'] }, 'economia': { 'nome': '💰 Economia', 'emoji': '💰', 'descricao': 'Preços, salários, negócios', 'cargos': \['CEO', 'Presidente', 'Master', 'Alta Cúpula', 'ADM staff', 'ADM'] }, 'eventos\_cultura': { 'nome': '🎭 Eventos e Cultura', 'emoji': '🎭', 'descricao': 'Festas, shows, atividades', 'cargos': \['CEO', 'Presidente', 'Resp Comunidade', 'Social Midia', 'Master', 'ADM staff'] }, 'regras\_leis': { 'nome': '⚖️ Regras e Leis', 'emoji': '⚖️', 'descricao': 'Leis da cidade, punições', 'cargos': \['CEO', 'Presidente', 'Equipe Legal', 'Master', 'Alta Cúpula', 'ADM staff', 'ADM'] }, 'sugestoes\_livres': { 'nome': '💡 Sugestões Livres', 'emoji': '💡', 'descricao': 'Qualquer ideia criativa', 'cargos': \['CEO', 'Presidente', 'Master', 'Alta Cúpula', 'ADM staff', 'Resp Comunidade', 'Mentoria', 'Equipe Legal', 'Social Midia', 'ADM'] } }

# Bot setup

intents = discord.Intents.default() intents.message\_content = True bot = commands.Bot(command\_prefix='/', intents=intents)

def init\_db(): conn = sqlite3.connect('feedbacks.db') c = conn.cursor()
...
class PainelPrincipal(discord.ui.View): """Painel com categorias - vai direto pro modal"""
...
class ModalFeedback(discord.ui.Modal): """Modal para escrever o feedback"""
...
class BotoesTicket(discord.ui.View): """Botões para controlar o ticket"""
...
class BotoesTicketResolvido(discord.ui.View): """Botões após ticket resolvido"""
...
class ConfirmarFechamento(discord.ui.View): """Confirmar fechamento do ticket"""
...
async def mostrar\_meus\_feedbacks(interaction): """Mostrar feedbacks do usuário""" print(f"📊 \[MOSTRAR FEEDBACKS] Iniciando para usuário {interaction.user}")
...
async def criar\_ticket\_privado(interaction, feedback\_id, categoria, texto\_completo, cargos\_responsaveis): """Criar thread privado para o feedback - VERSÃO CORRIGIDA"""
...
👤 **Cidadão:** {interaction.user.mention} 🎯 **Cargos notificados:** {' '.join(mentions)}

**Este é um espaço privado para discussão deste feedback.** Apenas os participantes podem ver e responder aqui.

{interaction.user.mention}, aguarde! Em breve algum responsável irá atendê-lo. **Staff**, respondam diretamente aqui para atender o cidadão! 📞"""
...
async def enviar\_dms\_fallback(interaction, feedback\_id, categoria, texto\_completo, cargos\_responsaveis): """Fallback: enviar DMs se thread não funcionar"""
...
@bot.event async def on\_ready(): print(f'✅ {bot.user} está online!') print(f'🔧 Sistema CORRIGIDO e com controle de tickets!') print(f'📊 Servidores conectados: {len(bot.guilds)}')
...
@bot.tree.command(name="feedback", description="🎮 Abrir painel de feedback") async def feedback\_command(interaction: discord.Interaction): # Verificar se é cidadão if not any(role.id == CARGO\_CIDADAO for role in interaction.user.roles): await interaction.response.send\_message("❌ **Acesso Negado!** Apenas cidadãos podem usar o sistema de feedback.", ephemeral=True) return
...
@bot.tree.command(name="dashboard", description="📊 Ver dashboard de feedbacks (Staff only)") async def dashboard\_command(interaction: discord.Interaction): # Verificar se é staff user\_role\_ids = \[role.id for role in interaction.user.roles] is\_staff = any(role\_id in CARGOS\_STAFF.values() for role\_id in user\_role\_ids)
